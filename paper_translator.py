...
# ──────────────────────────────────────────────────────────────────────────
def call_llm(client, model, system, user, max_tokens=8000, json_mode=False):
    cfg = types.GenerateContentConfig(
        system_instruction=system,
        max_output_tokens=max_tokens,
        # JSON 모드: 역슬래시 등 특수문자를 안전하게 이스케이프한 유효 JSON만 반환
...
def translate_blocks(blocks, client, model, cache, do_caption=True):
    ...
    for n, bt in enumerate(batches, 1):
        ...
        except Exception as e:  # noqa: BLE001  한 배치가 깨져도 전체는 계속
            print(f"  ! 배치 {n} JSON 파싱 실패({e}) → 해당 블록은 원문 유지", file=sys.stderr)
            translated = {}
        for key, txt in bt:
            ko = translated.get(str(key)) or txt   # 번역 없으면 원문으로 폴백
            result[key] = ko
